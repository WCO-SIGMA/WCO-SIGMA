...
# --- CONFIGURACIÓN DE CONEXIONES (URLs ACTUALIZADAS) ---
...
def generar_pdf_final(nit, an_c, pl_c, an_h, pl_h):
    ...
    sections = [("1. ANALISIS DE CONDICIONES", an_c, pl_c), ("2. ANALISIS DE COMPORTAMIENTO", an_h, pl_h)]
    for tit, an, pl in sections:
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 10, tit, ln=True)
        pdf.set_font("Arial", "", 10)
        pdf.multi_cell(0, 7, f"DIAGNOSTICO: {an}\nPLAN DE ACCION: {pl}")
        pdf.ln(5)
...
if not nit_user:
    ...
    st.info("Conexión establecida. Ingrese el NIT para activar el Dashboard Gerencial.")
else:
    ...
    if menu == "📊 Dashboard Gerencial":
        ...
        with t1:
            if not df_cond_e.empty:
                ...
                st.markdown("---")
                txt_an_c = st.text_area("Análisis Técnico (Condiciones):", key="an_c_f")
                txt_pl_c = st.text_area("Plan de Acción (Condiciones):", key="pl_c_f")
        ...
        with t2:
            if not df_comp_e.empty:
                ...
                txt_an_h = st.text_area("Análisis Conductual:", key="an_h_f")
                txt_pl_h = st.text_area("Estrategia Humana:", key="pl_h_f")
        ...
        with t3:
            ...
            if not df_acpm_e.empty:
                ...
                st.error("❌ No se detectan datos de ACPM para este NIT en la URL proporcionada.")
                if not df_acpm_t.empty:
                    with st.expander("Diagnóstico de Datos"):
                        st.write("Columnas detectadas:", list(df_acpm_t.columns))
                        st.write("Muestra de NITs en BD:", df_acpm_t['Nit_M'].unique()[:5])

        st.sidebar.markdown("---")
        if st.sidebar.button("📄 Procesar PDF"):
            pdf_b = generar_pdf_final(nit_user, txt_an_c, txt_pl_c, txt_an_h, txt_pl_h)
            st.sidebar.download_button("⬇️ Descargar Reporte", data=pdf_b, file_name=f"Reporte_{nit_user}.pdf", mime="application/pdf")
    ...
    # --- FORMULARIOS (VARIABLES RESTAURADAS) ---
        ...
        with st.form("f_acpm"):
            ...
            with col2:
                ...
                f_cambio = st.radio("⚠️ ¿Activa Gestión del Cambio?", ["No", "Sí"])
            if st.form_submit_button("💾 GUARDAR ACPM"):
                ...
                st.success("ACPM Registrada con éxito.")
        ...
        with st.form("f_comp"):
            ...
            if st.form_submit_button("🚀 REGISTRAR"):
                n_h = pd.DataFrame([{"Nit":str(nit_user), "ID_Inspección":str(uuid.uuid4())[:8].upper(), "Fecha/Hora Real":str(datetime.now()), "Inspector":f_in, "Centro de trabajo":f_cth, "Lugar":f_lgh, "Tipo de Inspección":"Conducta", "Estado observado":f_est, "Evidencia Fotográfica":"N/A", "Observaciones Factor Humano":f_obs}])
