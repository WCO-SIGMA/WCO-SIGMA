...
URL_ACPM = "PEGA_AQUI_LA_URL_DE_TU_BD_ACPM" 
...
if not nit_user:
    ...
    st.success("✅ Conexión con Motores BDI (SST, PESV, ACPM) Activa.")
    st.info("Ingrese su NIT para visualizar sus tableros exclusivos.")
else:
    ...
    if menu == "📊 Dashboard & Análisis":
        st.title(f"📊 Panel Gerencial SIG - NIT: {nit_user}")
        ...
        with tab1:
            if not df_cond_emp.empty:
                ...
                st.text_area("Análisis Técnico:", key="an_c_hoy")
        ...
        with tab2:
            if not df_comp_emp.empty:
                ...
                st.text_area("Diagnóstico de Factor Humano:", key="an_h_hoy")
        ...
        with tab3:
            if not df_acpm_emp.empty:
                ...
                with ca1: st.plotly_chart(px.pie(df_acpm_emp, names='Componente', title="ACPM por SIG", hole=0.4), use_container_width=True)
                with ca2: st.plotly_chart(px.bar(df_acpm_emp, x='Estado', color='Fuente', title="Avance de Tareas"), use_container_width=True)
                st.subheader("⚖️ Evaluación de la Mejora Continua")
                st.text_area("Análisis de Eficacia ACPM:", key="an_m_hoy")
    ...
    # --- PANTALLA 2: GESTIÓN DE ACPM (RESTAURADA) ---
        ...
        with st.form("form_acpm_vfinal"):
            ...
            with col1:
                ...
                st.subheader("Análisis de Causa")
            ...
            if st.form_submit_button("💾 GUARDAR ACPM"):
                ...
                conn.update(spreadsheet="BD ACPM", data=pd.concat([df_acpm_total, nueva], ignore_index=True))
    ...
    # --- REPORTES DE INSPECCIÓN ---
        ...
        with st.form("f_comp"):
            ...
            if st.form_submit_button("🚀 REGISTRAR"):
                ...
                conn.update(spreadsheet="BDI COMPORTAMIENTO", data=pd.concat([df_comp_total, nueva_comp], ignore_index=True))
