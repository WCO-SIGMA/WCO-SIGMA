   ...
   # --- PANTALLA 1: DASHBOARD GERENCIAL ---
    if menu == "📊 Dashboard Gerencial":
        ...
        with tab3:
            ...
            if df_acpm_t.empty:
                st.error("❌ El sistema no logra leer el archivo BD ACPM. Verifica la URL.")
            elif df_acpm_e.empty:
                st.warning(f"⚠️ No hay datos de ACPM para el NIT: {nit_user}")
                with st.expander("🔍 Diagnóstico de Columnas"):
                    st.write("Columnas detectadas:", list(df_acpm_t.columns))
                    if 'Nit' in df_acpm_t.columns:
                        st.write("NITs disponibles en el Excel:", df_acpm_t['Nit'].unique()[:5])
            else:
                ...
                st.markdown("---")
                st.text_area("Evaluación de la Mejora Continua / Plan de Acción ACPM:", key="an_m_vfinal")
